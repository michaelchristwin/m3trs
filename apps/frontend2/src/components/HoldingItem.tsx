...
export function HoldingItem({
  ...
  const { data: metadata, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const difference = Number(metadataRaw[3]) - now
      const status = {
  ...
  const isMoreThanAWeekAway = useMemo(() => {
    return metadata?.status.isMoreThanAWeekAway ?? true
  }, [metadata])
  return (
    <div
    ...
    >
      ...
      <div className="col-span-1 font-mono-data text-sm text-on-surface md:text-right">
        ...
        <span className="font-mono-data"> {metadata?.supply}</span>
      </div>
